...
const GoldenEditor = ({ data, onFavoriteAdd, onFavoriteRemove, onFavoriteUpdate }: { data: Notion; onFavoriteAdd: onFavoriteAdd; onFavoriteRemove: onFavoriteRemove; onFavoriteUpdate: onFavoriteUpdate }) => {
        ...
        createNewTask(
            ...
            },
            {
                onSuccess: (res) => {
                    const { id } = res;
                    setNotion((_notion) => {
                        const newTask = { id, checked: false, isFavorite: false, subTask: isSubTask, value, visible: false };
                        return {
                            ..._notion,
                ...
                <Droppable droppableId={notion.id} type="group">
                    {(provided) => (
                        <div className="h-[91%] overflow-auto scroll-bar" {...provided.droppableProps} ref={provided.innerRef}>
                            ...
                            <TextareaAutosize ref={addTaskRef} placeholder="Write New Dimension" style={{ whiteSpace: "pre-wrap", resize: "none", paddingLeft: `${isSubTask ? "50px" : "0px"}` }} className="placeholder-placeholder bg-transparent overflow-hidden  outline-none text-[14px] w-[90%] mr-[6px]" onKeyDown={onNewDimensionHandler} />
                        </div>
                    )}
                </Droppable>
            </DragDropContext>
