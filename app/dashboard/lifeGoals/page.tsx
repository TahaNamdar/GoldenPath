...
export default function LifeGoals() {
    ...
    for (let i = 0; i < limit; i++) {
        chipsFromAgeArray.push(<Chip key={i} counter={i + 1} daysLeft={days} age={years} index={i} chips={state[i + 1]} />);
    }
    ...
    const onDragEnd = (result: any) => {
        ...
        const item = sourceValues.Chips.find((item: any) => item.id === draggableId);
            ...
            <div className="flex-1  bg-CharlestonGreen rounded-md lg:pl-[3.7rem] lg:pr-[4.6rem] h-screen xl:h-auto">
                ...
                <main className="mb-[4rem]">
                    ...
                    <div className="bg-Crayola pl-[2rem]  lg:pr-[3.8rem] lg:pl-[3.8rem] lg:rounded-b-[1.4rem] ">
                        <section className="pt-[2rem] sm:pr-[2rem] pb-[2rem] ">{years === 0 ? <p className="text-white text-xl">You Should Set Your Birthday From Setting At First</p> : <DragDropContext onDragEnd={onDragEnd}>{chipsFromAgeArray}</DragDropContext>}</section>
                    </div>
                </main>
            </div>
        </div>
